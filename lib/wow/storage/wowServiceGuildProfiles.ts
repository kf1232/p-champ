...
/** Guild name, realm display name, and faction label for cache summary rows. */
export function extractGuildProfileSummaryStripFields(
  payload: unknown,
): { name: string; realmDisplay: string; factionName: string } {
  if (!payload || typeof payload !== "object") {
    return { name: "—", realmDisplay: "—", factionName: "—" };
  }
  ...
  if (realm && typeof realm === "object") {
    ...
    const rn = typeof r.name === "string" && r.name.length > 0 ? r.name : null;
    if (rn) realmDisplay = rn;
  }
  let factionName = "—";
