 ...
 * `MAIN_HAND` and legacy `WEAPON` are included; `OFF_HAND` is excluded (off-hands are
 * not enchantable under the same rules). BOOTS → FEET, RING 1/2 → FINGER_1/2,
...
/** Blizzard profile summary `character_class.id` / `active_spec.id` when present. */
export type ProfileClassSpecIds = {
  classId: number | null;
  activeSpecId: number | null;
};

export function extractProfileClassSpecIds(
  payload: unknown,
): ProfileClassSpecIds {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { classId: null, activeSpecId: null };
  }
  const p = payload as Record<string, unknown>;
  const cc = readNested(p, ["character_class"]);
  const spec = readNested(p, ["active_spec"]);
  return {
    classId: cc ? readNumber(cc, "id") : null,
    activeSpecId: spec ? readNumber(spec, "id") : null,
  };
}
...
/**
 * Guild realm slug + display name from a profile summary `guild` object, for Game Data
 * guild lookup (`/data/wow/guild/{realm}/{guild}`). Returns null if the character has no
 * guild or required fields are missing.
 */
export function pickCharacterGuildForGameDataLookup(
  profile: unknown,
): { guildRealmSlug: string; guildDisplayName: string } | null {
  ...
  return {
    guildRealmSlug: slug.trim(),
    guildDisplayName: name.trim(),
  };
}
